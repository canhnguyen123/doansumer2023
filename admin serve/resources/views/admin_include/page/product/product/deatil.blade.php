            ...
            <div class="table-agile-info">
                <div class="panel panel-default row">
                                    ...
                                    @foreach ($product_deatil_quantity as $item_quantity)
                                        <div class="item-req">
                                            <div class="item-res-pro">
                                                <p>Color</p>: <p class="size-color-Pro">
                                                    {{ $item_quantity->quantity_color }} </p>
                                            </div>
                                            <div class="item-res-pro">
                                                <p>size</p>: <p class="size-item-Pro">{{ $item_quantity->quantity_size }}
                                                </p>
                                            </div>
                                            <div class="item-res-pro">
                                                <p>SL</p>: <p class="quantyti-item-Pro">{{ $item_quantity->quantity_sl }}
                                                </p>
                                            </div>
                                        </div>
                                    @endforeach
                    ...
                    <div class="col-12 selec-commet">
                        <div class="content-list-commet" id="list-cmt-ajax">
                            @foreach ($list_cmt as $item_cmt)
                            @if ($item_cmt->user_id !== null)
                                <div class="item-commet" data-id="{{ $item_cmt->mess_parent_comment_id }}">
                                    <!-- Your HTML code for displaying comments of users (where user_id is not null) -->
                                    <a href="{{ route('user_deatil', ['user_id' => $item_cmt->user_id]) }}">
                                        <div class="img-user flex_center">
                                            <img src="{{ $item_cmt->user_img ? $item_cmt->user_img : 'https://firebasestorage.googleapis.com/v0/b/loco-7d8c6.appspot.com/o/c6e56503cfdd87da299f72dc416023d4.jpg?alt=media&token=0f06f1ca-d5a1-48e8-a8e7-704fdca9f927' }}" alt="">
                                        </div>
                                    </a>
                                    <div class="infro-user">
                                        <div class="content-cmt">
                                            <a href="{{ route('user_deatil', ['user_id' => $item_cmt->user_id]) }}">
                                                <div class="infro-user-name">{{ $item_cmt->user_fullname }}</div>
                                            </a>
                                            <div class="infro-user-text">{{ $item_cmt->cmt_text }}</div>
                                            <div class="reply-cmt-post">
                                                <p class="reply-cmt-post-text" data-id="{{ $item_cmt->cmt_id }}">Trả lời</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="post-commet flex_center">
                            <form action="" method="post" class="flex_center">
                                <input type="hidden" id="input-post-commet-reply" >
                                <input type="text" id="input-post-commet" placeholder="Nhập nội dung bình luận"
                                    required>
                                <button id="btn-post-commet" data-id="{{ $product_id }}">Bình luận</button>
                            </form>
                        </div>
                    </div>
