...
class admincontroller extends Controller
{
    ...
    public function post_login(Request $request)
    {
        ...
        if ($user) {
            // if (Hash::check($password, $user->staff_password)) {
               if (md5($password) === $user->staff_password) {
                ...
                return redirect()->route('login')->with('mess', 'Sai mật khẩu');
            }
        } else {
            return redirect()->route('login')->with('mess', 'Sai tên đăng nhập');
        }
    ...
    public function updatePassword(validateRequet $request, $id)
{
    ...
    public function updateShowHome(Request $request)
    {
        ...
        // Count the selected categories
        $selectedCount = count($theloaiItem);
